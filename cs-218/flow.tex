...
\subsection{Applications}
	...
	\subsubsection{Maximum Bipartite Matching}
		...
		\begin{proof}
			...
			Therefore, $M$ is a matching. Using \Cref{lem: max bi match} on the cut $(X\cup\{s\},Y\cup\{t\})$, $|f| = f^\to(S) - f^\ot(S) = |M|$, proving the required.
		\end{proof}
...
\subsection{Exercises}
	...
	\begin{solution*}
		...
		Using \Cref{ff-algo lemma 1}, $f$ must saturate $\vv{uv}$ since it is an edge from $S$ to $T$. At the same time, it must avoid this edge since it is an edge from $T'$ to $S'$, resulting in a contradiction and proving the claim.
	\end{solution*}
	...
	\end{exercise}
	\begin{solution*}
		We prove the required for $(S\cup S', T\cap T')$, the other side follows similarly. Let $f$ be a max-flow. Any edge from $S\cup S'$ to $T\cap T'$ must be an edge from $S$ to $T\cap T'$ (and thus $T$) or an edge from $S'$ to $T\cap T'$ (and thus $T'$). In particular, using \Cref{ff-algo lemma 1}, $f$ saturates every edge from $S\cup S'$ to $T\cap T'$. On the other hand, any edge from $T\cap T'$ to $S\cup S'$ must be an edge from $T$ to $S$ or $T'$ to $S'$, so $f$ avoids it. It follows that the cut must be a min-cut (because $|f|$ is equal to the capacity of the cut).
	\end{solution*}
