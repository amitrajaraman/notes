...
For each $i$, denote by $\mathsf{OPT}(v,i)$ the minimum weight path from $v$ to $t$ that uses at most $i$ edges. We want to compute $\mathsf{OPT}(s,n-1)$ (because all cycles are negative cycles).\\
