...
Since we have assumed that integer multiplication takes constant time, computing all the $C(\alpha_j)$ given $A(\alpha_j)$ and $B(\alpha_j)$ takes only $\mathcal{O}(n)$ time.\\
...
Observe that $A(x)=A_0(x^2)+xA_1(x^2)$ and the degree of each is around half the degree of $A$. Also observe that the values of $A_0(x)$ and $A_1(x)$, we can calculate $A(x)$ in a constant number of operations. 
...
Suppose we iterate backwards and we are currently and index $j$. Then note that if the $j$th job is in an optimal solution $\var{OPT}$, then no job $i$ with $i>p(n)$ cannot belong to $\var{OPT}$. If $j\not\in\var{OPT}$, then we can recurse on $\{1,\ldots,j-1\}$.\\
...
\begin{enumerate}
	...
	\item Decide on the memoization strategy. This can be done in the fly since $\texttt{cost}(i,j,k)$ can be computed in $\mathcal{O}(1)$ time. We recursively store the computed values of $\texttt{para}(i,j)$, starting from small intervals and building up to larger intervals.

	\item Check that the problems are acyclic. There's nothing much to argue here, the $\texttt{para}$ routine uses smaller intervals for larger intervals.
...
Therefore, let us restrict ourselves to directed \textit{acyclic} graphs for now.\\

Can we apply Djikstra's algorithm directly? No, we cannot. Consider the graph with $V=\{a,b,c\}$, $E=\{ac,ab,bc\}$, $s=a$, $t=c$, and $w=\{ac\mapsto 1, ab\mapsto 2, bc\mapsto-5\}$.
