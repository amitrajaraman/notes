...
% 	It is easy to see that any interval is contained in the union of these intervals. Let $\mathcal{A}=\{x_1,\ldots,x_k\}$ be the resulting set of indices chosen intervals and $\mathsf{OPT}=\{y_1,\ldots,y_m\}$ be an optimal solution.
% 	% We wish to show that $m\geq r$ ($=$ instead of $\geq$, in fact).
% 	It suffices to consider the first case of the algorithm (Why? It splits into smaller disjoint problems). \\ 
