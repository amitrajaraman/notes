...
\begin{solution*}
	...
	Assume without loss of generality that the jobs are ordered in non-increasing order of $t_2$. We claim that then, the identity permutation suffices. Let $k\in[n]$ attain the maximum described above. Let $\sigma$ be any permutation of $[n]$. Denote by $Q_\sigma$ the quantity above and by $Q$ the corresponding quantity for the identity permutation.

	\textbf{Claim.} If $Q_\sigma\leq Q$, then for every $1\leq i\leq k$, $1\leq\sigma(i)\leq k$.\\
