...
\subsection{Power series representation of analytic functions}
	...
	\end{ftheo}
	\begin{remark}
		By components, we mean maximal connected subsets of $G$.\\
		Since $\{\gamma\}$ is compact, $\{ z : |z| > R \} \subseteq G$ for sufficiently large $R$, so $\gamma$ has precisely one unbounded component.\\
	\end{remark}
	\begin{proof}
		Define $f : G \to \C$ by $f(a) = n(\gamma;a)$.
		If we manage to show that $f$ is continuous on $G$, we are done since the image of this map is a subset of the integers and each component is connected by definition.\\
		Recall that components of $G$ are open. Fix $a \in G$ and let $r = d(a ; \{\gamma\}) > 0$. If $|a-b| < \delta < r/2$, then
		\begin{align*}
			|f(a) - f(b)| &= \frac{1}{2\pi} \left| \int_\gamma \left( \frac{1}{z-a} - \frac{1}{z-b} \right) \dif z \right| \\
				&= \frac{1}{2\pi} \left| \int_\gamma \frac{a-b}{(z-a)(z-b)} \dif z \right| \\
