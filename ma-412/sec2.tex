...
\subsection{Power series representation of analytic functions}
	...
	\end{ftheo}
	\begin{proof}
		Suppose that $|f(z)| \le M$ for all $z \in \C$. We shall show that $f'(z) = 0$ for all $z \in \C$. By \nameref{theo: cauchy's estimate}, since $f$ is analytic on any disk $B(z,R)$, $|f'(z)| \le M/R$. However, $R$ is arbitrary so $f'(z) = 0$ for any $z \in \C$.
	\end{proof}

	\begin{ftheo}[Fundamental Theorem of Algebra]
		If $p$ is a non-constant polynomial, then there exists $a \in \C$ with $p(a) = 0$.
	\end{ftheo}
	\begin{proof}
		Suppose $p(z) \ne 0$ for all $z \in \C$. Consider the entire function $f(z) = 1/p(z)$. This function is then bounded as $p(z)$ goes to $\infty$ as $z$ goes to infinity. By \nameref{liouville's theorem}, $f$ (and thus $p$) is constant, which is a contradiction.
	\end{proof}