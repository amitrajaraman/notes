	...
	\subsection{Szemer\'{e}di's Regularity Lemma}
		...
		\subsubsection{The Result}
			...
			\begin{ftheo}[Szemer\'{e}di's Regularity Lemma]
				Given $0 < \varepsilon < 1$, there exists $M$ such that for $n\gg 0$, any graph $G_n$ admits a vertex partition $\mathcal{P} = V_0 \sqcup V_1 \sqcup \cdots \sqcup V_k$, where
				\begin{itemize}
					\item $k \le M$,
				...
				\end{itemize}
			\end{ftheo}
			Such a partition is often referred to as an \textbf{$\varepsilon$-regular partition}.\\
			...
			To prove this, we use an ``energy increment'' argument.
			...
			Partitioning increases energy! Also observe that since the random variable $Z$ is at most $1$, so is the energy.\\

			\begin{lemma}
				If $(U,W)$ is $\varepsilon$-irregular, there is a partition $\mathcal{U} = \{U_1,U_2\}$ and $\mathcal{W} = \{W_1,W_2\}$ of $U,W$ respectively such that
