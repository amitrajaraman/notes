\section{Fundamental Results in Extremal Graph Theory}
	...
	\subsection{Szemer\'{e}di's Regularity Lemma}
		...
		\subsubsection{Interesting corollaries}
			...
			How is this related to Roth's Theorem? Given $N\gg 0$ and $A\subseteq[N]$, suppose $A$ is $3$-AP free. Let $M = 2N+1$ and construct a $3$-partite graph $G$ whose three parts $X$, $Y$, and $Z$ are copies of $\Z_M$.\\
			For $x\in X$, $y\in Y$, and $z\in Z$, keep an edge from $x$ to $y$ iff $y-x \in A$, $y$ to $z$ iff $z-y \in A$, and $z$ to $x$ iff $(z-x)/2 \in A$. The final part is well-defined since $2$ is invertible in $\Z_M$ (Why?). \\
			Observe that if $xyz$ is a triangle with $a = y-x$, $b = z-y$, and $2c = z-x$, then $a+b=2c$, so $a$,$c$,$b$ are in AP.\\
			Since $a$, $b$, and $c$ are all in $A$, then by our assumption we must have $a=b=c$, $y-x=z-y=(z-x)/2$, and so $x$, $y$, $z$ must be in AP in $\Z_M$.\\
			...
			To get a subset $A$ from these points, project $x = (a_1,\ldots,a_d)$ to $\sum_{j=1}^d a_j (2k+1)^{j-1}$, thus converting it to an integer in base $(2k+1)$. Observe that if $x_1 \mapsto m_1$ and $x_2 \mapsto m_2$, then $(x_1 + x_2) \mapsto (m_1 + m_2)$. As a result, if $X$ is a subset of $S$ on a single sphere, its projection by this map gives a $3$-AP free subset.\\
			...
			for some constant $c$. This is $\Omega(N^{1-\delta})$ for any (fixed) $\delta$!