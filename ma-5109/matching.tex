	...
	\subsection{Flow}
		...
		\subsubsection{Algorithms for finding max-flow}
			...
			\begin{itemize}
				...
				\item In fact, in the case where capacities are irrational and we choose an arbitrary path at each step, we need not even converge to the max-flow value.\footnote{If the capacities are integral (or rational), this process terminates as seen in the proof of \Cref{integral network integral maxflow}.}
			\end{itemize}
