...
Given a graph, how do we compute a maximum matching, or simpler yet, $\nu(G)$?
	...
	\subsection{Flow}
		...
		\subsubsection{The Max-Flow Min-Cut Theorem}
			...
			\begin{fcor}
				Suppose $\mathcal{N}$ is an integral network. That is, $C(e) \in \N_0$ for all $e \in E(D)$. Then, there is a polynomial time algorithm that determines an integral maximum flow $f$ on $\mathcal{N}$ (a flow which has maximum value and only takes integral values).
			\end{fcor}
			\begin{proof}
