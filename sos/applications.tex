...
\subsection{The bipartite support case}
	...
	Interestingly, the exact constant of approximation is an open problem, and the interested reader may look up \emph{Grothendieck's inequality} for more details. The approximation constant is known to be somewhere between $\pi/2 \approx 1.57$ and $\pi/(2\ln(1+\sqrt{2})) \approx 1.78$.\\

	Since $\supp(B)$ is bipartite, there exists some partition $X \cup Y$ of $[n]$ such that $B_{xx'} = B_{yy'} = 0$ for any $x,x' \in X$ and $y,y' \in Y$.