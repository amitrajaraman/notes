...
\subsection{The bipartite support case}
	...
	% explain cut again, introduce normalized cut/conductance \varphi_G(S) E(S,S')/(d/n)|S||S'|. denominator is expected number of edges in (d/n) Erdos R\'{e}nyi. \varphi_G = \min \varphi_G(S) = conductance/expansion/uniform sparsest cut problem.
	% explain the bottle-neck sort of thing pr[going out of S from random vertex in S]. Given $G$, it turns out that computing $\varphi_G$ is NP-hard (try as exercise, redn from max-cut)
	% [Chawla-Kranthgamar-Kumar-Rabani-Sivakumar] UGC implies no const-factor approximation for \varphi_G. pretty crazy, since we can get const-factor approximation for max-cut and exact min-cut in poly.
	% possible to show that \E_S \varphi_G(S) \ge 1/2... but conductance can be very small, even 0 if disconnected.
	% \varphi_G = min_{S \subseteq V, 0 < |S| < n} ... = min_{x in {-1,1}^n} (f_G(x))/(d/n(sum_{i,j} (x_i - x_j)^2)/4) 
	% equivalently, find \alpha such that f_G(x) - \alpha (d/4n) sum_{i,j} (x_i - x_j)^2 <--- get SoS cert for this
	...
	% in earlier secs, we did some sort of local analysis where we just comapred \pE_\mu (x_i - x_j)^2 and \E_{\mu'} (x_i - x_j)^2. here, we shall do global analysis instead.
