\section{Expander graphs and applications}
	...
	\subsection{Lectures 7, 8, 10: Testing connectivity of undirected graphs}
		...
		\subsubsection{Lecture 10}
			...
			\begin{equation}
				\label{eqn: zigzag spectral gap}
				\gamma(G\zigzag H) \ge \gamma_1 \gamma_2^2.
			\end{equation}
		
			Now, how do we construct the expander graph $H$?\\
			...
			It may be shown using \Cref{eqn: zigzag spectral gap} without much trouble that $\lambda(G \zigzag H) \le \lambda_1 + 2\lambda_2$.\\
			...
			Before coming to the proof of \Cref{eqn: zigzag spectral gap}, we describe the construction used to convert $G$ to a graph $G'$ satisfying the conditions described earlier.\\
			\begin{algorithm}[H]
				\DontPrintSemicolon
				\SetNoFillComment
				\SetKwProg{Fn}{}{}{}
			...
			% The only thing that remains is to prove \Cref{eqn: zigzag spectral gap}.

			\begin{proof}[Proof of \Cref{eqn: zigzag spectral gap}]
				...
				\textbf{Claim.} Let $C$ be a random walk matrix with eigenvalue $\lambda$. Then, it is possible to write $C = (1-\lambda)J + \lambda E$ for some matrix $E$ with spectral norm\footnote{the largest absolute value of an eigenvalue} bounded from above by $1$.\\
				Indeed,
				\[ \norm{E} = \frac{1}{\lambda} \norm{C - (1-\lambda) J} \ge \frac{1}{\lambda} \left( \norm{C} - (1-\lambda)\norm{J} \right) = 1. \]
				...
				Now, we wish to bound the second eigenvalue of $M$. Note that $\tilde{J}\tilde{A}\tilde{J}$ is precisely the random walk matrix of $G \otimes K_n$, which has second eigenvalue equal to that of $G$. Recalling very carefully that this is attained for some vector $v$ orthogonal to $\textbf{1}$, we have
				\begin{align*}
