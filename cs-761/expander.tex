\section{Expander graphs and applications}
	...
	\subsection{Lecture 7: Testing connectivity of undirected graphs}
		...
		\subsubsection{Lecture 8}
			...
			Suppose we want $\norm{p^{(t)} - u_1} \le \epsilon \coloneqq 1/n^2$. For our specific choice of $p^{(0)}$ concentrated on a single point, it is easily verified that $\norm{p^{(0)} - u_1} \le 1$. Therefore, for $t > \log_{\lambda} \epsilon$, $\norm{p^{(t)} - u_1} \le \epsilon$.\\
			We have that the convergence time is
			\[ \log_\lambda \epsilon = O\left( \frac{1}{1-\lambda} \log \epsilon \right). \]
