\section{Expander graphs and applications}
	...
	\subsection{Lecture 11: Reed-Solomon Codes}
		...
		\subsubsection{Lecture 11}
			...
			\end{ftheo}
			This says that over large fields, they essentially match the best possible rate-distance tradeoff.
			...
			This is just a linear systme of $n$ equations. If $D+d > n$, which is indeed true, we may solve it for a non-trivial solution.\\
			...
			\begin{ftheo}
				If there exists a polynomial $h \in \F[x]$ of degree at most $k-1$ such that $d_H(\Enc(h),r) < (n-k+1)/2$, the Welsh-Berlekamp algorithms outputs it.
			\end{ftheo}
			\begin{proof}
