\section{Expander graphs and applications}
	...
	\subsection{Lectures 6--7: Magical graphs and two applications}
		...
		\subsubsection{Lecture 6}
			...
			\end{definition}
			\begin{definition}
				Given $x,y \in \{0,1\}^n$, the \emph{Hamming distance} $d_H(x,y)$ between $x$ and $y$ is $|\{i \in [n] : x_i \ne y_i\}|$ and the \emph{relative distance} $\Delta(x,y)$ between $x$ and $y$ is $d_H(x,y)/n$. The distance $d_H(\mathcal{C})$ of a code $\mathcal{C}$ is $\min_{\substack{x,y \in \mathcal{C} \\ x \ne y}} d_H(x,y)$.
			\end{definition}
