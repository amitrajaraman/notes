...
\end{theorem}
	...
	Suppose there is no such $S$. Define a model as follows. Assign all the literals of an unassigned scc $1$ if all its children (the vertices it has an edge to in the reduced implication DAG) are assigned $1$. Consequently, assign its complement $0$. One of the directions of the result follows.\\
...
A \textit{Horn clause} is a cluase of the form $\neg p_1\lor \cdots\lor\neg p_n \lor q$, where $p_1,\ldots,p_n\in\Vars$ and $q\in\Vars\cup\{\bot\}$. A \textit{Horn formula} is a conjunction of Horn clauses. The clauses with $\bot$ literals are called \textit{goal clauses} and the others are called \textit{implication clauses}. Note that an horn clause is equivalent to
