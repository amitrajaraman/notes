\subsection{Symmetric functions}
	...
	\begin{proof}
		...
		Because monomials of different degree do not interact, we may assume that $\deg(\restr{m_j}{y_i = e_i}) = d$ for all $j$ for some $d$. Therefore,
	...
	\begin{proof}
		...
		\end{enumerate}
		In the first two cases, we say that the occurences of $i,(i+1)$ are ``paired''. In the third case, we say that the occurence is free. Suppose that some row of $F$ has $a$ $i$s and $b$ $(i+1)$s. Note that these $(a+b)$ occurrences are consecutive! Showing this requires the fact that $F$ is an SSYT.\\
	...
	Therefore, $S_\lambda$ is a symmetric function, known as the \emph{Schur symmetric function}.