\subsection{Symmetric functions}
	...
	Finally, we define another total order over number partitions of a fixed number.
	...
	\[ m_2 m_1 = m_3 + m_{2,1}. \]
	For $\lambda \vdash d$ with $\lambda = \lambda_1,\cdots,\lambda_\ell$, where $\ell \ge 2$ (we have already looked at the case where $\lambda = d$). Consider
	...
	\end{corollary}
	\begin{proof}
		The first part is precisely what we showed in the previous proposition. For the second part, we have that if $\lambda \pge \mu$, then $\mu^* \pge \lambda^*$, so $\lambda^* \ge_{\text{revlex}} \mu^*$.
	\end{proof}
	...
	\end{fcor}
	\begin{proof}
		By \Cref{lem: invertible M 1}, the diagonal elements of $M$ are all equal to $1$. By \Cref{lem: invertible M 2,gale-ryser}, $M$ is upper triangular. Therefore, $M$ is invertible. As it has determinant $1$, it has an integral inverse.
	\end{proof}
	...
	\begin{proof}
		...
		If $f$ has two monomials $m_1,m_2$ such that $\deg(\restr{m_1}{y_i = e_i}) \ne \deg(\restr{m_1}{y_i = e_i})$, we immediately have a contradiction. Therefore, the monomials of $f$ are such that $\deg(\restr{m_j}{y_i = e_i}) = d$ for all $j$ for some $d$. Therefore,
	...
	\end{ftheo}
	\begin{proof}
		Let $E(t) = \sum_{n \ge 0} e_n t^n$ be the ogf of the $e_i$, and similarly $H(t) = \sum_{n \ge 0} h_n t^n$. By \Cref{eqn: prod 1 m xi yj,eqn: prod 1 p xi yj}, we have that
		\begin{align*}
	...
	Let us look at one last class of symmetric functions, known as the \emph{power sum symmetric functions}. For $n > 0$, define
	...
	\[ \left( \sum_{i_1 \ge 1} x_{i_1}^{\lambda_1} \right) \cdot \left( \sum_{i_\ell \ge 1} x_{i_\ell}^{\lambda_\ell} \right). \]
	...
	\begin{proof}
		Let $(S_1,\ldots,S_k)$ be an ordered partition of $[\ell]$ such that for each $1 \le r \le k$, $\sum_{j \in S_r} \lambda_j = \mu_r$.\\
		Let $i \in S_{p_i}$ for each $i \in [\ell]$. Then, for any $1 \le r \le \ell$,
		\[ \sum_{i=1}^r \lambda_{i} \le \sum_{i=1}^r \mu_{p_i} \le \sum_{i=1}^r \mu_{i}, \]
		so $\mu \pge \lambda$.
	\end{proof}
	...
	\[ c_\lambda = \mathfrak{n!}{\prod_{i \ge 1} i^{m_i} m_i!} = \frac{n!}{z_\lambda}. \]
	...
	\begin{proof}
		Let
		\[ m_\lambda = \sum_{\varphi \vdash d} A_{\lambda\varphi} u_\varphi \text{ and } h_\mu = \sum_{\varphi \vdash d} B_{\mu\theta} v_\theta. \]
		Now,
		\begin{align*}
		...
		Suppose that $\{u_\lambda\},\{v_\mu\}$ form a pair of dual basis, so $T = I_{p(d)}$, so $I = AB^\top = B^\top A = A^\top B$. That is,
	...
	\begin{proof}
		...
		\begin{enumerate}
			...
			which is non-negative and equal to $0$ iff all $A_\lambda$ are $0$.
			\item 
		\end{enumerate}
	\end{proof}
