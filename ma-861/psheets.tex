...
\section{Problem Sheets}
	...
	\subsection{Problem Sheet 3}
		...
		\begin{solution*}
			...
			% Using Theorem 5.1.4 in \cite{ec2} wtih $g(k) = \indic_{k = 2}$,
			% \[ E(x) =  \]
			% \begin{align*}
			% 	E_{n+1} &= \sum_{k=1}^{n} \binom{n}{k} E_k E_{n-k}. \\
			% 	\frac{E_{n+1}}{(n+1)!} &= \frac{1}{n+1} \sum_{k=1}^n \frac{E_k}{k!} \frac{E_{n-k}}{(n-k)!}
			% \end{align*}
			% Now, observe that the coefficient of $x^n$ in $E(x)^2$ is $\sum_{k=0}^n \frac{E_k}{k!} \frac{E_{n-k}}{(n-k)!}$. Denoting this coefficient by $F_n$,
			% \begin{align*}
			% 	\frac{E_{n+1}}{(n+1)!} &= \frac{1}{n+1} \left(F_n - \frac{E_n}{n!}\right) \\
			% 	\sum_{n \ge 0} \frac{E_{n+1}}{(n+1)!} x^{n+1} &= x \sum_{n \ge 0} \frac{1}{n+1} \left(F_n - \frac{E_n}{n!}\right) x^n  \\
			% \end{align*}
		\end{solution*}
