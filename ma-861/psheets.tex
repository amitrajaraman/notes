\section{Problem Sheets}
	...
	\subsection{Problem Sheet 3}
		...
		% 	\phantom{pain}
		% 	\begin{enumerate}[label=(\alph*)]
		% 		\item Let $V(P_n) = [n]$ with $u \leftrightarrow v$ iff $u-v \in \{-1,1\}$. If a matching $M$ on $P_n$ has the edge $e = \{n-1,n\}$, $M \setminus \{e\}$ is a matching of size $|M|-1$ on the subgraph induced by $[n-2]$. If $M$ does not contain $e$, then it is a matching of size $|M|$ on the subgraph induced by $[n-1]$. Therefore,
		% 		\[ m_{k}(P_n) = m_{k-1}(P_{n-2}) + m_{k}(P_{n-1}) \]
		% 		and we have
		% 		\begin{align*}
		% 			\Match_{P_n}(x) &= \sum_{k \ge 0} m_{k}(P_n) x^k \\
		% 				&= \sum_{k \ge 0} x \cdot m_{k-1}(P_{n-2}) x^{k-1} + m_{k}(P_{n-1})x^k = x\Match_{P_{n-2}}(x) + \Match_{P_{n-1}}(x).
		% 		\end{align*}

		% 		\item Let $V(C_n) = \Z_n$ with $u \leftrightarrow v$ iff $u-v \in \{-1,1\}$. If a matching $M$ on $P_n$ has the edge $e = \{n-2,n-1\}$, $M \setminus \{e\}$ is a matching of size $|M|-1$ on the subgraph induced by $\Z_n \setminus \{n-2,n-1\}$, which is isomorphic to $P_{n-2}$. If $M$ does not contain $e$, then it is a matching of size $|M|$ on the subgraph $C_n \setminus \{e\}$, which is isomorphic to $P_{n}$. Therefore, using the recurrence from (a),
		% 		\begin{align*}
		% 			m_{k}(C_n) &= m_{k-1}(P_{n-2}) + m_{k}(P_{n}) \\
		% 				&= m_{k-2}(P_{n-4}) + {\color {darkgray} m_{k-1}(P_{n-3})} + m_{k-1}(P_{n-2}) + {\color{darkgray} m_{k}(P_{n-1})} \\
		% 				&= m_{k-1}(C_{n-2}) + {\color{darkgray} m_{k}(C_{n-1})}.
		% 		\end{align*}
		% 		and as in the case of $P_n$, we have
		% 		\[ \Match_{C_n}(x) = x\Match_{C_{n-2}}(x) + \Match_{C_{n-1}}(x). \]

		% 		\item Let $V(K_n) = [n]$. If a matching $M$ on $K_n$ has an edge $e$ incident on $n$ (note that if this is the case, then precisely one such edge can be in $M$), then $M \setminus \{e\}$ is a matching of size $|M|-1$ on the subgraph induced by $[n] \setminus \{e\}$, which is isomorphic to $K_{n-2}$. On the other hand, if no such edge is in $M$, then $M$ is a matching of size $|M|$ on $K_{n-1}$. Therefore,
		% 		\[ m_k(K_n) = (n-1) m_{k-1}(K_{n-2}) + m_{k}(K_{n-1}). \]
		% 		So,
		% 		\begin{align*}
		% 			\Match_{K_n}(x) &= \sum_{k \ge 0} m_{k}(K_n) x^k \\
		% 				&= \sum_{k \ge 0} x \cdot (n-1)m_{k-1}(K_{n-2}) x^{k-1} + m_{k}(K_{n-1})x^k = (n-1)x\Match_{P_{n-2}}(x) + \Match_{P_{n-1}}(x).
		% 		\end{align*}
		% 	\end{enumerate}
		% \end{solution*}
		...
		\end{problem}
		% \begin{solution*}
		% 	We already saw in the previous problem that $\Match_{C_n}(2x)$ satisfies the same recurrence as $2T_n(x)$. It is easily verified that the two are equal for $n=1,2$, so we are done.
		% \end{solution*}
