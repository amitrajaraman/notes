...
\section{Problem Sheets}
	...
	\subsection{Problem Sheet 3}
		...
		\end{problem}
		\begin{solution*}
			The above is easily verified for $n=1,2$, and we saw in \Cref{problem: three term recurrence matching} that $\Match_{C_n}$ satisfies the same recurrence as $2T_n$, which is the same as that of $T_n$ \Cref{prop: recurrence of Tn}.
		\end{solution*}
		...
		\end{problem}
		% \begin{solution*}
		% 	Denote by $\mathsf{Rook}_{n,n-k}$ the desired quantity. Let $P$ be a placement of $r$ non-attacking rooks. Suppose that in $P$, there are $0 < t \le r$ rooks on the diagonal cells of the form $(i,i)$. Observe that the unattacked positions now correspond to $\mathsf{St}_{n-t}$. That is, the placement of the remaining $n-t$ rooks can be thought of as a placement on $\mathsf{St}_{n-t}$. If there are no rooks on the diagonal, the placement is just one on $\mathsf{St}_{n-1}$. As a result, we have
		% 	\[ \mathsf{Rook}_{n,r} = \mathsf{Rook}_{n-1,r} + \sum_{t=1}^{r} \binom{n}{t} \mathsf{Rook}_{n-t,r-t}. \]
		% 	We take $\mathsf{Rook}_{k,0} = 1$. 
		% \end{solution*}