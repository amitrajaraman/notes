...
This inequality is slightly loose. The best possible constant on the right has an extra multiplicative factor of $2$ and was proved in \cite{dyer-frieze-sample-outside}. We omit the proof of the above and later prove a better bound in \Cref{improvement of conductance isoperimetric inequality}. We shall now use this to show that the conductance of our random walk is large.\\
...
\begin{proof}
	...
	The above implies that if we show uniform boundedness of the $(\psi_i)$ on $[s,t]$, then (uniform) equicontinuity follows as well. And indeed, since the $(\psi_i)$ are concave, then letting $\psi_i$ attain its supremum in $[s,t]$ and $x_i$, we have 
...
The above result has numerous interesting corollaries which are of interest to us. The first is an improvement (Why?) of \Cref{conductance isoperimetric inequality}.
...
The above bound is tight when $K$ is a cylinder and $F$ is identically $1$.
...
\begin{proof}
	...
	\begin{equation}
		\label{eqn: local lemma in cond iso ineq improvement}
	...
	Substituting $g$ and $h$ in \Cref{eqn: local lemma in cond iso ineq improvement}, we get
	\begin{equation}
	...
	\end{equation}
	We aim to get a contradiction to the above. Intuitively, the worst case appears to be when $H_3$ is a single interval -- even in general, we can reduce it to this case as we shall see later. Let us prove the following, which just asserts that \Cref{eqn: cond iso ineq imp contradictory eqn} is incorrect. Define $\mu_G$ to be measure on $[0,1]$ with density $G$.
