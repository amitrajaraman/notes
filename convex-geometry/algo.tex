...
\begin{proof}
	It suffices to consider the case where $g$ and $h$ are continuous. Indeed, if not, we can find some monotone (strictly) increasing sequence of continuous integrable functions $(g_k)$ and $(h_k)$ that converge to $g$ and $h$. We then have
	...
	Further, note that it suffices to show the inequality with a $\geq 0$ instead of $>0$. Indeed, we can then apply it to $(g-a)$ and $(h-a)$ for some function $a$ that is everywhere positive and continuous with a sufficiently small integral to get the strict inequality.\\
	We complete the rest of the proof over a series of lemmas. 
\end{proof}
...
\begin{proof}
	...
	% Now, we claim that the functions $\psi_i$ are uniformly equicontinuous on any interval $[0,1]$ with $0<s<t<1$. Indeed, we see that they are all continuous on $[0,1]$, which is a compact space, so they are all uniformly continuous.
	Observe that the $(\psi_i)$ are uniformly bounded since
	\[ \vol(K_i) \geq \frac{\vol_{n-1}(K_i\cap Z_t)}{2}(\beta_i-\alpha_i) \geq \frac{\vol_{n-1}(K_i\cap Z_t)}{2}. \]
	Further, the $(\psi_i)$ are equicontinuous (Why?).\\
	Therefore, by the Arzel\'a-Ascoli Theorem, the $(\psi_i)$ have a uniformly convergent subsequence; let $\psi$ be the resulting limit function. Clearly, $\psi$ is non-negative, concave, and
...
\begin{proof}[Continued Proof.]
	...
	The body $K'$ can be visualized more naturally as taking the union of the $(n-1)$-dimensional simplex spanned by $te_1, te_1+\psi(t)e_2,\ldots,te_1+\psi(t)e_n$ over all $0\leq t\leq 1$. As a result,
	\[ \vol_{n-1} (K'\cap Z_t) = \frac{\psi(t)^{n-1}}{n!}, \]
	where $Z_t=\pi_1^{-1}(t)$ is defined as in the previous lemma.\\ 
	Then observe that
	\[ \int_{K'} \hat{g}(x)\d{x} = \int_0^1 \vol(K' \cap Z_t) g(t e_1) \d{t} = \frac{1}{n!} \int_0^1 \psi(t)^{n-1} g(t) \geq 0, \]
	...
	Since we've taken $\norm{a-b}$ to be minimal, we may assume wlog that $\int_{K'}\hat{g}(x)\d{x}=0$.\\
	...
	\[ \int_{L_A}\hat{g}(x)\d{x} = \int_{L_A'}\hat{g}(x)\d{x} = 0 \]
	and 
	\[ \int_{L_A}\hat{h}(x)\d{x} \geq 0. \]
	Note that $L_A$ must intersect both $Z_0$ and $Z_1$, since otherwise, the minimality of $\norm{a-b}$ is contradicted. Therefore, $H$ cannot be orthogonal to the $x_1$-axis and it can be described as $x_2+\cdots+x_n=\ell(x_1)$ for some linear function $\ell$.
\end{proof}


% \subsection{An \texorpdfstring{$\mathcal{O}^*(n^7)$}{} Algorithm using the Ball-Step}

% As in nearly all volume estimation algorithms, the basic idea remains the same, the changes being only in the walk. The algorithm used here involves the ``ball-step'' mentioned earlier. It is worth noting instead of the ``ball'' in ball-step, one could use any other symmetric convex body $G$. An obvious choice is the cube, which is quite convenient to draw points uniformly randomly from from a programming perspective. Our analysis shall be done using this general case.\\
