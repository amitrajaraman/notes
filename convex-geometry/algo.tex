...
\begin{ftheo}
	...
	\end{equation}
	Then the $(7s/t)$-conductance of $\mathcal{M}(K,G)/F$ is at least $t^2\theta/6$.
\end{ftheo}

% *** They have proved t^2\theta / 18 ???
...
Right around now, the reader might be wondering why we even introduced this alternate way to obtain a sandwiching; it does not seem to be any better than what we did earlier with the strong satisfaction of the second condition. In special cases however, this method turns out to be extremely powerful.\\
...
Suppose otherwise. Then at least $s+1$ of the $2s+1$ $\zeta_i$ are outside the interval. We can then use Chernoff's inequality, which yields that out of $2s+1$ independent events, each having a probability at most $1/3$ of occurring, the probability that more than half occur is at most $e^{-s/10}\leq\delta$.
