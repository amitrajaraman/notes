...
\begin{align*}
	M 	&= \sup_{A} \frac{Q_0(A)}{Q_F'(A)} \\
		...
		&= \frac{\mu_F(q B_2^n)}{\mu_F(B_2^n)} \\
		&\leq \frac{n!\vol(K)}{e^{-3\sqrt{n}\vol(B_2^n)}} & (\text{\Cref{eqn: 4.21}}) \\
		&\leq n^{4n} \left(n!\leq n^n, \frac{\vol(K)}{\vol(B_2^n)} \leq \frac{3}{2}m^n\right, e^{-3\sqrt{n}}\leq\frac{2}{3}n^{n/2})
\end{align*}
for sufficiently large $n$. By \Cref{lem 4.46},
\[ \Phi \geq \frac{1}{20000q\sqrt{n}}. \]
Therefore,
\[ |Q_t(U) - Q_F'(U)| \leq n^{2n}\left(1-\frac{\Phi^2}{2}\right)^t \]