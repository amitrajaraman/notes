...
\begin{proof}
	...
	\begin{align*}
		\int_{S_1} \int_{S_2\cap (x+G)} &\geq \frac{t}{6}\vol(G)\mu_F(S_1'\cup S_2') \\
			...
			&\geq \frac{t^2\theta}{18} 
	\end{align*}
	% *** wtf @ the last line
	%  bruh do I even need s-conductance
\end{proof}
...
\end{lemma}
\begin{proof}
	Set $h(t)=e^{-t}/(n-1)!$ and fix some $0\leq s\leq 1$. Observe that the set $sK$ is mapped by $H$ to $s\lambda(s)K$. Obviously, the probability of $s\lambda(s)K$ under the uniform distribution (on $K$ is $(s\lambda(s))^n$. The probability of $sK$ under the given density $h$ is
...
There exists a randomized algorithm to obtain a constant of $n$ in the above, but for now we shall analyze it for a general $m$.\\

Set $q=2mn\log(4/\varepsilon)$ and $t=10^{10}nq^2(n\log n + \log(2/\varepsilon))$.\\
In the walk, we start from the uniform distribution on $B_2^n$ and do a lazy random walk in $qB_2^n$ up till time $t$ with $B_2^n$-steps, filtered by $F$ (as described earlier in the section). Compute $w=H(v_t)$. Our goal is to see how close the distribution $H(v_t)$ is to the uniform distribution. The main result of this section is the following.
...
Note that it implies that $n^{-1/2}/3 B_2^n \subseteq K$. Also, \Cref{lemma 4.38} implies that $K\subseteq 3nmB_2^n$. Therefore, for any $x\in K$,
...
\end{lemma}
\begin{proof}
	Let $\overline{K}=K\cap mB_2^n$. Note that $F_{\overline{K}}(x) < F(x)$. We then have
	\begin{align*}
		\int_{2nmB_2^n} F(x)\d{x} &\geq \int_{2n\overline{K}} F(x)\d{x} \\
			&\geq \int_{2n\overline{K}} \overline{F}(x)\d{x} \\
			&= n\vol(\overline{K}) \int_0^{2n} e^{-t}t^{n-1}\d{t}
	\end{align*}
\end{proof}