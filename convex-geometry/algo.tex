...
The definition of $h_k$ is similar to a continuous analogue of the fractional knapsack problem wherein the sum weights correspond to integration with respect to $Q$, the sum of values corresponds to integration with respect to $Q_k$, and the weight capacity is $x$. This also lends itself very naturally in the proof of \Cref{hk distance supremum attained}, where we ``assign $g$'' to the points which have a high value-weight ratio -- this corresponds to the Radon-Nikodym derivative in the continuous case.\\
