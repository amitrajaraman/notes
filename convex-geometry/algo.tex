...
\begin{proof}
	...
	\begin{align*}
		\int_{S_1} \int_{S_2\cap (x+G)} &\geq \frac{t}{6}\vol(G)\mu_F(S_1'\cup S_2') \\
			...
			&= \frac{t^2\theta}{18}  \min\left\{\mu_F(S_1)-\frac{7s}{t}, \mu_F(S_2)-\frac{7s}{t}\right\}.
	\end{align*}
...
\begin{proof}
	...
	so $\phi(y)\leq\phi(x)+1$. Therefore, $F_K(y)\geq F_K(x)/e$. This bounds the local conductance below by $1/9e > 1/25$. In the context of \Cref{cor to bound conductance}, the minimum value of $\theta$ is bounded below by $2/3pn^{1/2}$\footnote{try bounding the distance $d$ such that $\vol(B_2^n \cap (de_1 + B_2^n)) \geq \vol(B_2^n)/2$ and going from there.} (using \Cref{better bound on spherical caps}). \Cref{cor to bound conductance} then yields the result.
\end{proof}
