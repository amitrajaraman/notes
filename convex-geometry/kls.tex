...
\subsection{Recent Bounds on the Isoperimetric Constant}
	...
	% \subsubsection{An Overview}
		...
		\begin{equation}
			...
			\begin{gathered}
				...
				p_t(x) = \frac{e^{\langle c_t,x\rangle - (1/2)\norm{x}_{B_t}^2}p(x)}{\int_{\Rn}e^{\langle c_t,y\rangle - (1/2)\norm{y}_{B_t}^2}p(y)\d{y}}, \;\;\; \mu_t(x) = \expec_{x\sim p_t} [x], \\	
			\end{gathered}
		\end{equation}
		...
		\begin{align*}
			d{A}_t &= \int_{\Rn} (x-\mu_t)(x-\mu_t)^\top \cdot (x-\mu_t)^\top\d{W}_t \cdot p_t(x) \d{x} - A_t^2 \d{t} \\
				&= \expec_{x \sim p_t} (x-\mu_t)(x-\mu_t)^\top \cdot (x-\mu_t)^\top\d{W}_t - A_t^2 \d{t}.
		\end{align*}
		...
		Over the next few sections, we give a $n^{-1/4}$ bound on $\psi_p$, as described in \cite{lee-vempala-kls-n14}. More precisely, we show that $\psi_p \gtrsim \Tr(A^2)^{-1/4}$.
	...
	\subsubsection{Controlling \texorpdfstring{$A_t$}{At}}
		...
		\begin{proof}
			...
			\begin{align*}
				\norm{\expec_{x \sim p}\left[(x-\mu)(x-\mu)^\top C (x-\mu)\right]}_2 &= \norm{\expec_{y\sim \tilde{p}}\left[A^{1/2}y(y^\top w)^2\right]}_2 \\
					...
					&= \max_{\norm{\zeta}_2 \leq 1} \sqrt{\zeta^\top A \zeta}
					&\lesssim \opnorm{A}^{1/2} \norm{w}_2^2.
			\end{align*}
		...
		Before we get onto the proof, we show that the bound on the isoperimetric constant follows from previously mentioned results.
		...
		\begin{proof}
			...
			\[ \Pr\left[\max_{s \in [0,t]} \rho(p_s) \leq \sqrt{8 \Tr(A_0^2)}\right] \geq 0.99. \]
			Therefore, letting
			\[ T = \min\left\{c_1, \frac{1}{64\sqrt{8}}\right\} \frac{1}{\sqrt{\Tr(A_0^2)}}, \]
			we have
			\[ \Pr\left[\int_0^T \rho(p_s)\d{s} \leq \frac{1}{64}\right] \geq 0.99, \]
		...
		\begin{proof}[Proof of \Cref{lee-vem: growth theorem}]
			...
			\[ \delta_t \lesssim \Tr(A_t^2)^{3/2} \lesssim \Phi_t^{3/2} \]
			...
			Let $\tilde{W}_t$ be a martingale such that $Y_t = \tilde{W}_{[Y]_t}$ in distribution. Using \Cref{reflection principle},
			\begin{align*}
			...
			\[ \Pr\left[\max_{t\in[0,T]} \Phi_t \geq 8\Phi_0 \right] \leq 2\exp(-8) \leq 0.01. \]
		\end{proof}
	...
	\subsubsection{An Almost Constant Bound}
		...
		Here, we prove that there is a constant $c$ such that for any log-concave $p$ in $\Rn$ and integer $\ell \geq 1$,
		\[ \psi_p \geq \frac{1}{(c\ell(\log n + 1))^{\ell/2} n^{16/\ell} \sqrt{\rho(p)}}. \]
		In particular, setting
		\[ \ell = \left\lceil \sqrt{\frac{\log n}{\log \log n}} \right\rceil, \]
		there exists a constant $c'$ such that
		\[ \psi_p \geq \frac{1}{n^{c'\sqrt{\log \log n / \log n} \sqrt{\rho(p)}}}. \]
		For this, the control matrix $C_t$ in \Cref{eqn: main stoch local} as $A^{-1}$. The resulting stochastic differential equations are then
		\begin{equation}
		...
		For starters, for any integer $n \geq 1$, define
		\[ \psi_n = \inf_{\substack{\text{$p$ is log-concave in $\R^n$} \\ \text{and has compact support}}} \psi_p\sqrt{\rho(p)}. \]
		...
		\end{equation}
		Using \cite{Paouris2006}, we may assume that the log-concave density $p$ has compact support -- we may restrict it to a sufficiently large ball losing exponentially small measure in the process. We can in fact get away with much looser constraints in this situation and restrict ourselves to a ball throwing away at most $0.2$ of the measure.\\

		Assume the dimension $n$ is at least $3$.

		To control the spectral norm, we give two results.
		Define the potential
		\[ Q_t = A^{-1/2} A_t A^{-1/2}, \;\;\; \Gamma_t = \Tr(Q_t^q). \]
		It is quite easy to see that $\Gamma_t^{1/q} \geq \norm{Q_t}_2$
		\begin{lemma}
			With the above definition,
			\begin{equation}
				\label{kls chen bound A}
				\d{\Gamma_t} \lesssim \frac{2q^2}{t}\Gamma_t\d{t} + (\text{martingale term})
			\end{equation}
			and
			\begin{equation}
		...
		\end{theorem}
		\begin{proof}
			Since it is difficult to control the eigenvalue directly, define the potential function
			\[ Q_t = A^{-1/2} A_t A^{-1/2}, \;\;\; \Gamma_t = \Tr(Q_t^q). \]
			Clearly, $\Gamma_t \geq \norm{A^{-1/2} A_t A^{-1/2}}_2$. Let
			\[ T_1 = \frac{1}{c'q\alpha^2\log(n)n^{2\beta}} \text{ and } T_2 = \frac{n^{\beta/4q}}{40}T_1, \]
			where $c' = 32768$.

			It follows from \eqref{kls chen bound A} that
			\[ \expec[\d{\Gamma_t}] \lesssim \frac{2q^2}{t} \expec[\Gamma_t\d{t}], \]
			so for $t_2 > t_1 > 0$, we have
			\begin{equation}
				\label{label: expectation of potential A}
				\expec[\Gamma_{t_2}^{1/q}] \leq \expec[\Gamma_{t_1}^{1/q}] \left(\frac{t_2}{t_1}\right)^{2q}.	
			\end{equation}
			On the other hand, it follows from \eqref{kls chen bound B} that
			\begin{equation}
				\label{label: expectation of potential B}
				\Pr\left[\max_{t\in[0,T_1]} - \frac{1}{(1+\Gamma_t)^{1/q}} \geq -\frac{1}{2(n+1)^{1/q}} \right] \leq \exp\left(-\frac{2}{3}q\log n\right) \leq \frac{3}{10}.
			\end{equation}

			We bound the integral spectral norm by doing so separately on $[0,T_1]$ and $[T_1,T_2]$.
			\begin{itemize}
				\item[$[0,T_1]$] 

				\item[$[0,T_2]$] We have
				\begin{align*}
					\expec\left[\Gamma_t^{1/q}\right] &\leq \expec\left[\Gamma_{T_1}^{1/q}\right] \left(\frac{t}{T_1}\right)^{2q}
				\end{align*}
			\end{itemize}
		\end{proof}

		Combining \eqref{chen norm bound} and \eqref{eqn: chen prob in terms of spectral norm}, we have
		...
		Substituting $T_2$, we get that for the conditions of \Cref{chen norm bound},
		\begin{equation}
			\label{eqn: main bootstrap}
			\psi_n \geq \frac{1}{cq^{1/2}\alpha\log(n)^{1/2}n^{\beta - (\beta/8q)}}.
		\end{equation}
		It may also be checked that this holds for dimensions $1$ and $2$. How do we extend this to a general bound on the isoperimetric constant?
		...
		\begin{proof}
			...
			By the bound \Cref{isoperimetric coefficient bound 1} in the original KLS paper, it follows that for all $n\geq 1$,
			...
			As mentioned at the beginning of this section, the second part easily follows on setting
