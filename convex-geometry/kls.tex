...
Next, we shall show the counterpart of \Cref{lem: 5.1} for exponential needles.
...
\begin{proof}
	...
	\[ \left(\int_0^{|b-a|} f_1(a+tu) F(a+tu)\ell(a+tu)\ell(a+tu)\right)^\alpha \left(\int_0^{|b-a|} f_2(a+tu) F(a+tu)\ell(a+tu)\ell(a+tu)\right)^\beta > \left(\int_0^{|b-a|} f_3(a+tu) F(a+tu)\ell(a+tu)\ell(a+tu)\right)^\alpha \left(\int_0^{|b-a|} f_4(a+tu) F(a+tu)\ell(a+tu)\ell(a+tu)\right)^\beta. \]
	However, $F\ell^{n-1}$ is concave, so 
\end{proof}