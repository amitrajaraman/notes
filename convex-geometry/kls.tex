...
\begin{proof}
	...
	\begin{itemize}
		...
		The inner expression goes to $1/e$. If $\gamma'=\infty$, then we again get $\ell_n(t)^n\to 0$ for $t < b_0$, so we arrive at a contradiction similar to the first case above.\\
		Otherwise, we have
...
\begin{proof}
	...
	\begin{itemize}
		...
		We also have
		\begin{align*}
			1 &= \frac{1}{\vol(K)} \int_K (v^{\circ \top} x)^2 \d{x} \\
			 &=  \frac{1}{\vol(K)} \int_{\partial B_2^n} \int_0^{\varphi(u)} t^{n-1} (v^{\circ \top} (v+tu))^2 \d{t}\d{u} \\
			 &= \frac{1}{\vol(K)} \int_{\partial B_2^n} \left( \frac{\varphi(u)^n}{n}\norm{v}^2 + 2\frac{\varphi(u)^{n+1}}{n+1} (v^{\circ \top}u) + \frac{\varphi(u)^{n+2}}{n+2} (v^{\circ \top}u)^2 \right) \d{u} \\
			 &= \frac{1}{\vol(K)} \int_{\partial B_2^n} \left( \frac{\varphi(u)^n}{n} \left(\frac{\sqrt{n(n+2)}}{n+1}\norm{v} + \sqrt{\frac{n}{n+2}}\varphi(u)(v^{\circ \top} u)\right)^2 + \frac{\varphi(u)^n}{n(n+1)^2}\norm{v}^2 \right) \d{u} \\
			 &= \frac{1}{\vol(K)} \left( \int_{\partial B_2^n} \frac{\varphi(u)^n}{n} \left(\frac{\sqrt{n(n+2)}}{n+1}\norm{v} + \sqrt{\frac{n}{n+2}}\varphi(u)(v^{\circ \top} u)\right)^2 \d{u} \right) + \frac{\norm{v}^2}{(n+1)^2}
		\end{align*}
		...
		\[ \left( \frac{1}{\vol(K)} \int_{\partial B_2^n} \frac{\varphi(u)^n}{n} \left(\frac{\sqrt{n(n+2)}}{n+1}\norm{v} + \sqrt{\frac{n}{n+2}}\varphi(u)(v^{\circ \top} u)\right)^2 \d{u} \right) \cdot 1 \geq \frac{1}{(n+1)^2n(n+2)}\norm{v}^2. \]
		That is,
...
For any convex body, let $\alpha(K)$ be the largest eigenvalue of $A(K)$.

\begin{ftheo}
	For any convex body $K\subseteq\Rn$,
...
\begin{theorem}
	Let $K$ be a convex body in $\Rn$ and assume that $b(K)=0$. Let $u\in\Rn$ have unit norm and $\beta=\expec_K((u^\top x)^2)$. Then
	\[ \vol(K \cap \{x : u^\top x < 0\}) \vol(K \cap \{x : u^\top x > 0\}) \geq \frac{1}{10}\sqrt{\beta}\vol(K)\vol_{n-1}(K \cap \{x : u^\top x = 0\}). \]
\end{theorem}