...
\[ p_t(x) = \frac{e^{\langle c_t,x\rangle - (t/2)\norm{x}^2}p(x)}{\int_{\Rn}e^{\langle c_t,y\rangle - (t/2)\norm{y}^2}p(y)}\text{ and } \mu_t(x) = \expec_{x\sim p_t} [x]. \]
The reason we insist on the density being more log-concave than a Gaussian is that this often leads to quite nice bounds on the isoperimetric constant $\psi_p$ using the \nameref{localization lemma}.\\
However, to add another method to control the covariance, we often add a \textit{control matrix} $C_t$, to control the covariance $A_t$ of the density $p_t$ at time $t$. This is incorporated into the previous equations as
\begin{equation}
	\label{eqn: main stoch local}
	\begin{gathered}
		c_0 = 0, \;\;\; \d{c}_t = C_t^{1/2}\d{W}_t + C_t\mu_t\d{t}, \\
		B_0 = 0, \;\;\; \d{B}_t = C_t\d{t}, \\
		p_t(x) = \frac{e^{\langle c_t,x\rangle - (t/2)\norm{x}_{B_t}^2}p(x)}{\int_{\Rn}e^{\langle c_t,y\rangle - (t/2)\norm{y}_{B_t}^2}p(y)}, \;\;\; \mu_t(x) = \expec_{x\sim p} [x], \\	
	\end{gathered}
\end{equation}
...
It is also not too difficult to show using It\^{o}'s Lemma that \eqref{eqn: main stoch local} results in \eqref{eqn: continuous time stochastic localization density}.\\

Now, since we are interested in the covariance, we would like to determine its growth. To do this,
\begin{aligned*}
	\d{\log p_t(x)} &= (x-\mu_t)^\top \d{W}_t - \frac{1}{2} (x-\mu_t)^\top (x-\mu_t) \d{t} \\
		&= x^\top (\d{W}_t + \mu_t\d{t}) - \frac{1}{2}\norm{x}^2\d{t} + g(t) \\
		&= x^\top\d{c}_t - \frac{1}{2}\norm{x}^2\d{t} + g(t),
\end{aligned*}
where $g(t)$ is independent of $x$ and the first two terms. This explains the appearance of the Gaussian in \eqref{eqn: main stoch local}.\\
...
Over time, the Gaussian factor in the definition of $p_t$ begins to dominate.\\