...
\Cref{kls conj hyperplane} and the \nameref{con: kls conjecture} together assert that up to a constant factor, a hyperplane cuts is the ``worst'' cut involved in the isoperimetric coefficient.\\

\cite{chen2021constant} has made the most progress in recent times, proving that there is a constant $c$ such that
\[ \psi(K) \geq \frac{1}{\sqrt{ d^{c\left(\log\log d/\log d\right)^{1/2}} \alpha(K)}}. \]