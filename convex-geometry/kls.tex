...
\subsection{A More Detailed Look}
	...
	\subsubsection{The Poincar\'{e} Constant}
		...
		It may be shown that $\zeta_p$ is within a constant factor of the isoperimetric constant $\psi_p$. Due to this strong relation, we shall in fact use the Poincar\'{e} constant in a later proof to bound the isoperimetric constant.
...
\subsection{Recent Bounds on the Isoperimetric Constant}
	...
	% \subsubsection{An Overview}
		...
		For now, let us discuss how stochastic localization works out in discrete time. Instead of multiplying by this indicator function, we multiply by an affine functional that is very close to $1$. That is, we transform the density $p(x)$ to $(1\pm\varepsilon\langle x-\mu,\theta\rangle)p(x)$ (this gives two probability measures), where $\mu$ is the barycenter of the measure and $\theta$ is randomly chosen. This is like a reweighting in favour of one of the half-spaces.\\
		The resulting measures are probability measures since we are merely reweighting. The average of these two measures is the original measure. Further, each of these measures are log-concave (the product of log-concave functions is log-concave)!\\
	...
	\subsubsection{Controlling \texorpdfstring{$A_t$}{At}}
		...
		Over the next few sections, we look at the best bound attained (as of the time of writing) of the isoperimetric constant, as described in \cite{chen2021constant}. The proof 
	...
	\subsubsection{An Almost Constant Bound}
		...
		Therefore, we wish to control the spectral norm. To do this, define the potential
		\[ \Gamma_t = \Tr(A_t^q). \]
		...
		\begin{proof}
			...
			for a suitable constant $c$.\\
			Denote the expression on the right by $\kappa^{-1}$.\\
			...
			Also, by the definition of $\psi_n$, the Poincar\'{e} constant is at least $c\psi_n^2 / \opnorm{A_t}$. So, $\zeta_{p_t} \geq \kappa$.\\
			Recalling \eqref{eqn: dAt expression}, let $\d{(A_t)_{i,j}} = \langle\xi_{i,j},\d{W}_t\rangle - (A_t^2)_{i,j} \d{t}$, where
			\[ \xi_{i,j} = \int_{\Rn} x x_i x_j p_t(x+\mu_t) \d{x}. \]
			...
			If $\lambda_i - \lambda_j = 0$, we interpret the corresponding term as $f''(\lambda_i)$. The $\xi_{i,j}$ are expressed in the basis of the eigenvectors.\\
			...
			\begin{align*}
				\delta_t &= q \left( \frac{1}{2} \sum_{i,j = 1}^n \norm{\xi_{i,j}}^2 \frac{\lambda_i^{q-1} - \lambda_j^{q-1}}{\lambda_i - \lambda_j} - \sum_{i=1}^n \lambda_i^{q+1} \right) \\
					&\leq q \left( \frac{1}{2} \sum_{i,j = 1}^n \frac{q-1}{2} \norm{\xi_{i,j}}^2 (\lambda_i^{q-2} + \lambda_j^{q-2}) - \sum_{i=1}^n \lambda_i^{q+1} \right) \\
		...
		\begin{proof}
			...
			proving the claim.\\
			The second part relating to $\expec[\Gamma_t^{1/q}]$ is easily obtained on using the fact that $\Gamma_t \geq n \opnorm{A_t}^q$.
		\end{proof}
		...
		\begin{proof}
			...
			So, for any $T_1 > T_0$,
			\begin{align*}
				\expec\left[\int_0^{T_1} \expec[\opnorm{A_t}] \d{t} \right] &\leq 3T_1 + 3n^{1/q} \expec\left[\int_{T_1}^{T_2} \left(\frac{t}{T_0}\right)^{2q} \d{t} \right] \\
					&\leq \frac{3}{100} + 3 \frac{ n^{1/q} }{2q+1} \frac{T_1^{2q+1}}{T_0^{2q}} \leq \frac{3}{100} + 3 n^{1/q} \frac{T_1^{2q+1}}{T_0^{2q}}.
			\end{align*}
			How high can we make $T_1$ while ensuring that the integral we care about (for the purposes of \Cref{chen basic estimate}) remains less than $1/8$?\\
			We get
