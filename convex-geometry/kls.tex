...
\subsection{Recent Bounds on the Isoperimetric Constant}
	...
	% \subsubsection{An Overview}
		...
		\begin{align*}
			p(\partial E) &= \expec\left[p_t(\partial E)\right] & (p_t\text{ is a martingale}) \\
				...
				&= \frac{1}{4}\norm{B_t^{-1}}_2^{-1/2}\Pr\left[\frac{1}{4} \leq p_t(E) \leq \frac{3}{4}\right]\min\{p(E),p(\Rn\setminus E)\}.
		\end{align*}
	...
	\subsubsection{Towards an Almost Constant Bound}
		...
		First of all, with a proof very similar to \Cref{basic estimate}, in this case we have
		\[ \Pr\left[\frac{1}{4} \leq p_t(E) \leq \frac{3}{4}\right] \geq \frac{9}{10} - \Pr\left[\int_0^t \norm{A^{-1/2}A_sA^{1/2}}_2\d{s} \geq \frac{1}{64}\right]. \]
		Using \cite{Paouris2006}, we may assume that the log-concave density $p$ has compact support -- we may restrict it to a sufficiently large ball losing exponentially small measure in the process. We can in fact get away with much looser constraints in this situation and restrict ourselves to a ball throwing away at most $0.2$ of the measure, but we state the above stronger result anyway.

		For any integer $d \geq 1$, define
		\[ \psi_d = \inf_{\substack{\text{$p$ is log-concave in $\R^d$ and has compact support}}} \psi_p\sqrt{\rho(p)}. \]

		To control the spectral norm, we give the following result.

		\begin{theorem}
			Let $p$ be a log-concave density with compact support and invertible covariance matrix $A$. Suppose that for some $0<\beta\leq 1/2$ and $\alpha \geq 1$, $\psi_k \geq 1/\alpha k^\beta$ for all $k \leq d$. Then, there exists a universal constant $c$ such that for $q = 1 + \lfloor 1/\beta \rfloor$, $d \geq 3$, and $T_2 = 1/cq\alpha^2 \log(d)d^{2\beta - (\beta/4q)}$,
