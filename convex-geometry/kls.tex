...
\subsection{Recent Bounds on the Isoperimetric Constant}
	...
	\subsubsection{Towards an Almost Constant Bound}
		...
		\begin{theorem}
			Let $p$ be a log-concave density with compact support and invertible covariance matrix $A$. Suppose that for some $0<\beta\leq 1/2$ and $\alpha \geq 1$, $\psi_k \geq 1/\alpha k^\beta$ for all $k \leq n$. Then, there exists a universal constant $c$ such that for $q = 1 + \lfloor 1/\beta \rfloor$, $d \geq 3$, and $T_2 = 1/cq\alpha^2 \log(d)d^{2\beta - (\beta/4q)}$,
