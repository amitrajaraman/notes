	...
	\subsubsection{An Almost Constant Bound}
		...
		\begin{lemma}
			...
			\begin{equation}
				...
				\d{\Gamma_t} \leq q^2\psi_n^{-2}\opnorm{A_t}\Gamma_t\d{t} + (\text{martingale term}).
			\end{equation}
		\end{lemma}
		\begin{proof}
		...
		\end{ftheo}
		\begin{proof}
			Since it is difficult to control the eigenvalues directly, define the potential function
			\[ Q_t = A^{-1/2} A_t A^{-1/2}, \;\;\; \Gamma_t = \Tr(Q_t^q). \]
			...
			\[ \expec[\d{\Gamma_t}] \leq \frac{2q^2}{t} \expec[\Gamma_t\d{t}], \]
			so for $t_2 > t_1 > 0$, we have
			\begin{equation}
				\label{eqn: expectation of potential A}
			...
			\begin{equation}
				\label{eqn: expectation of potential B}
			...
			\begin{itemize}
				\item[$\displaystyle{[0,T_1]}$] We have
					\begin{align*}
						\Pr\left[\int_0^{T_1} \Gamma_t^{1/q} \geq \frac{1}{128} \right] &\leq \Pr\left[\max_{t\in[0,T_1]} \Gamma_t^{1/q} \geq \frac{1}{128 T_1} \right] \\
							...
							&= \Pr\left[\max_{t\in[0,T_1]} - \frac{1}{(1+\Gamma_t)^{1/q}} \geq -\frac{1}{2}(n+1)^{1/q} \right] \\
				...
				So, every eigenvalue of $A_{T_1}$ is upper bounded by $1/T_1$ and $\Gamma_{T_1}$ is upper bounded by $n/T_1^q$.
				Then,
				\begin{align*}
		...
		Combining \Cref{chen norm bound} and \eqref{eqn: chen prob in terms of spectral norm}, we have
