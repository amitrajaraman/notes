...
\section{More on decompositions}
	...
	\subsection{Hyperplane bisections}
		...
		This serves as a measure of how ``imbalanced'' the hyperplane corresponding to $x$ is. Indeed, $f_{E,K}(x) = 0$ iff the hyperplane corresponds to $x$ is a bisecting hyperplane (where bisecting means that $\mu(E \cap K_\omega) = \frac{1}{2} \mu(K_\omega) $, as in needle decompositions).\\
		For nice(?) $E$, $f_{E,K}$ is continuous.\\

		The primary tool used in \cite{lov-sim-on7} to prove the localization lemma was showing that there exists a bisecting hyperplane passing through any $(n-2)$-dimensional affine space. How does this translate in terms of the above defined function?\\
		Suppose we have an $(n-2)$-dimensional affine space orthogonal to the subspace defined by $y,z\in\Rn$ and passing through $y$.\\
		Suppose that $x$ defines a hyperplane containing this affine space. Then $x$ is orthogonal to the plane, and so orthogonal to the space itself. That is, it must lie in the subspace spanned by $y$ and $z$. Further, $y-x$ is orthogonal to $x$. That is, the set of all these $x$ forms a circle $S$ passing through $0$ contained in the $2$-dimensional subspace spanned by $y,z$.\\
		The conclusion of the localization method is that either
		\begin{itemize}
			\item $f(w) = 0$ for some $w \in S \setminus \{0\}$ or
			\item The limit of $f(w)$ as $w$ goes to $0$ along the circle is equal to $0$ -- this corresponds to a bisecting hyperplane passing through the origin itself. It is not too difficult to check that this makes sense and that the directional limit along either direction of the circle is the same. 
		\end{itemize}

		More generally, suppose we have some one-dimensional smooth curve $C$ in $\Rn$ that passes through the origin. Then, as before, either $f(w) = 0$ for some $w \in C \setminus \{0\}$ or one of the directional limits at $0$ (along $C$) is equal to $0$.\\

		A consequence of this nice property is that given any non-zero $x \in \Rn$, there exists a one-dimensional ``barrier'' curve $C$ in $\Rn$ separating $x$ from $0$ such that $f(y) = 0$ for any $y \in C$.
	...
	\subsection{A potential function}
		...
		Also, as seen from \Cref{eqn 4}, all we really want is that $\mu_\omega(E)(1-\mu_\omega(E)) \gtrsim 1$, it might be fine to instead just minimize $f$ instead of ensuring that it is exactly equal to $0$. So, one may choose the $x$ corresponding to the bisecting hyperplane at each step by minimizing a potential function such as
