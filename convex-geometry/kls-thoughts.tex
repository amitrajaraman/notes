...
\section{More on Decompositions and a Potential}

	\subsubsection{Hyperplane Bisections}
		...
		That is, if $H$ is the hyperplane defined by $x$ (orthogonal to $x$ and passing through it) is $H$, the value of the above function at $x$ is equal to $|\mu(E \cap H^+) - \mu((\Rn\setminus E) \cap H^+)|$ where $H^+$ is either of the halfspaces defined by $H$.\\
		...
		Suppose we have an $(n-2)$-dimensional affine space orthogonal to the subspace defined by $y,z\in\Rn$ and passing through $y$. What is the set of hyperplanes passing through this space? Suppose that $x$ defines a hyperplane passing through this space. Then $x$ is orthogonal to the plane, and so orthogonal to the space itself. That is, it must lie in the subspace spanned by $y$ and $z$. Further, $y-x$ is orthogonal to $x$. That is, the set of all these $x$ forms a circle $S$ in the $2$-dimensional subspace spanned by $y,z$ passing through $0$.\\
		...
		More generally, suppose we have some one-dimensional smooth curve $C$ in $\Rn$ that passes through the origin. Then, as before either $f(w) = 0$ for some $w \in C \setminus \{0\}$ or one of the directional limits to $0$ is equal to $0$.
