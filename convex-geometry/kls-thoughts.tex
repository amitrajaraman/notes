...
\section{More on decompositions}
	...
	\subsection{Hyperplane bisections}
		...
		\[ \left| \int_{\{ z \in \Rn : \langle z , x \rangle \geq \norm{x} \}} p(y) ( \mu(\Rn\setminus E)\indic_E - \mu(E)\indic_{\Rn \setminus E} ) \dif y \right| \]
	...
	\subsection{Aligned 2-dimensional decompositions are always possible}
		...
		In general, given $k$ directions, it is possible to decompose a body $K$ into $\{K_\omega\}$ such that each of the $K_\omega$ is $(n-k)$-dimensional and each of them contains these $k$ directions.
