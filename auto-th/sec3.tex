...
\label{subsec: limits of computation}
	...
	\end{exercise}
	% if the lang generated by A (B) above is L_A (L_B) then PCP answer is yes iff L_A^c \cup L_B^c is all of \Sigma^*. a cfg for this can be created. 
	...
	Now, let us see a problem that is \textsf{NP}-complete.

	\begin{fprob}[The Satisfiability Problem]
	...
	\begin{proof}
		...
		\begin{enumerate}
			...
			\begin{itemize}
				...
				\item If $\delta(q,X)$ contains $(p,Y,R)$ or $(p,Y,L)$, and we have $CqX$ at some point in the ID, then it changes appropriately. The encoding of this is very similar to that done in \Cref{ftheo: pcp is undecidable}, so we omit it and skip the tedium. There is a special case to be considered at the ``edges'' of the ID.
			\end{itemize}
		\end{enumerate}
	\end{proof}
