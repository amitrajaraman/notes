...
\subsection{Compact Spaces}
	...
	% \end{exercise}
	% \begin{solution*}
	% 	Let $C$ be closed in $X\times Y$. Then $U = (X\times Y)\setminus C$ is open, so can be written as a union $U = \bigcup_{i\in I} U_i\times V_i$, where each $U_i,V_i$ is open in $X,Y$ respectively.
	% 	% Since the projection map is open, $\{\pi_1(U)\times V_i : i\in I\}$ forms an open cover of $U$.
	% \end{solution*}
