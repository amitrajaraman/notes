...
\section{Introduction}
	...
	\subsection{Regularisation}
		...
		\subsubsection{Understanding regularisation}
			...
			To reiterate, there are two sources of randomness everywhere: the dataset (we assume that there is some `nice' distribution over all possible datasets) and the noise, which are independent.\\
			...
			Fix some new input $\hat{x}$. The question we would like to answer is: what is the expected test error $\expec[(f(\hat{x}) - (g(\hat{x}) + \varepsilon))^2]$? The expectation is taken over both the dataset and the noise.\\
			Let $\hat{y} = g(\hat{x}) + \varepsilon$. We then have
			\begin{align*}
				\expec[(f_\mathcal{D}(\hat{x}) - \hat{y})^2] &= \expec[f_\mathcal{D}(\hat{x})^2] + \expec[\hat{y}^2] - 2 \expec[f_\mathcal{D}(\hat{x})]\expec[\hat{y}] & \text{(assuming $\mathcal{D}$ and $\varepsilon$ are independent)} \\
