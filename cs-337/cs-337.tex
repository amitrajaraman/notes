...
\section{Classification}
	...
	\subsection{Kernels}
		...
		What about when our dataset is infinite? Indeed, in this case we can extend it to an eigen\emph{function} decomposition and write
		\[ K(x_1,x_2) = \sum_{i=1}^\infty \alpha_i \phi_i(x_1) \phi_i(x_2), \]
		where each $\alpha_i \ge 0$ and $\sum_{i=1}^\infty \alpha_i^2 < \infty$. The condition that $\alpha_i \ge 0$ corresponds to the fact that the matrix is positive semi-definite (all eigenvalues are non-negative). This is known as the \emph{Mercer kernel} decomposition.\\
